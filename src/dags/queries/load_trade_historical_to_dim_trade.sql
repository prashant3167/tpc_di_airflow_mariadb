...
WITH expanded_trade AS (
    ...
    FROM
        ...
        JOIN staging.trade_history_historical th ON t.T_ID = th.TH_T_ID
),
recent_trade AS (
    SELECT
        T_ID,
...
FROM
    recent_trade t
    JOIN master.dim_date std ON std.DateValue = DATE(t.creation_time)
    JOIN master.dim_time stt ON PARSE_TIME(
        "%H:%M:%S",
        stt.TimeValue
    ) = TIME_TRUNC(TIME(t.creation_time), SECOND)
    LEFT JOIN master.dim_date en ON en.DateValue = DATE(t.closing_time)
    LEFT JOIN master.dim_time et ON PARSE_TIME("%H:%M:%S", et.TimeValue) = TIME_TRUNC(TIME(t.closing_time), SECOND)
    JOIN master.status_type sty ON t.T_ST_ID = sty.ST_ID
    JOIN master.trade_type tt ON t.T_TT_ID = tt.TT_ID
    JOIN master.dim_security sec ON t.T_S_SYMB = sec.Symbol
    AND DATE(t.creation_time) >= sec.EffectiveDate
    AND DATE(t.creation_time) < sec.EndDate
    JOIN master.dim_account acc ON t.T_CA_ID = acc.AccountID
    AND DATE(t.creation_time) >= acc.EffectiveDate
    AND DATE(t.creation_time) < acc.EndDate;